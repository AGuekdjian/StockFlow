...
test('admin registers an entry and sees the append-only movement', async ({ page }) => {
  ...
  await expect(page.getByLabel('Código interno')).toHaveValue('CAM-######');
  await page.getByLabel('Ubicación').selectOption({ index: 1 });
  await page.getByLabel('Conteo físico inicial').fill('3');
  await page.getByRole('button', { name: 'Crear producto' }).click();
  const productRow = page.getByRole('row').filter({ hasText: 'CAM-000002' });
  ...
  const updatedRow = page.getByRole('row').filter({ hasText: 'CAM-000002' });
  ...
  await expect(page.getByRole('row').filter({ hasText: 'CAM-000002' })).toContainText('Inactivo');
